...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            wte=nn.Embedding(config.vocab_size, config.n_embd),  # token 对应向量
            # 位置embedding,基于索引
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        print(
            f"num decayed parameter tensors: {len(decay_params)}, with {num_decay_params:,} params")
        print(
            f"num non-decayed parameter tensors: {len(nodecay_params)}, with {num_nodecay_params:,} params")
...
for step in range(max_steps):
    ...
    if master_process:
        print(
            f"step {step:4d} | loss: {loss.item():.6f} | lr: {lr:.4e} | norm: {norm:.4f} | dt: {dt*1000:.2f}ms")
        with open(log_file, "a") as f:
